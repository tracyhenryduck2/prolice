...
<html>    
  <head>  
	...
	<script type="text/javascript">    
		function add(){      
			Dialog.open({Title:"新增", Width:600, Height:360, URL:"<%=path%>/medical/Role!toAddRole.action"});   
		}      
		function mod(){      
			...
			Dialog.open({Title:"编辑", Width:600, Height:360, URL:"<%=path%>/medical/Role!toAddRole.action?oper=1&roleBean.id="+id});
		}      
		function detail(){   
			var id = getCheckRowValue();     
			if(id==null) {     
			...
			Dialog.open({Title:"详情", Width:500, Height:360, URL:"<%=path%>/medical/Role!toAddRole.action?read=1&oper=1&roleBean.id="+id});    
		}      
		function del(){      
			if(!isCheckAny("idArr")){      
			...
			Dialog.confirm("确定删除吗", function(){     
				...
				form1.action = "<%=path%>/medical/Role!delRoles.action";
				form1.target = "fram";
				form1.submit();  
		...
		function search() {  
			Dialog.open({Title:"查询", Width:650, Height:220, URL:"<%=path%>/medical/Role!search.action",OnLoad:function(){
  ...
  <body>  
  <GF:BodyCaption label="职位维护" ico="images/ico/user.gif"> 
  	<GF:ToolBar id="123">     
    ...
    <form action="<%=path%>/medical/Role!list.action" name="form1" id="form1" method="get">  
   
			<input type="hidden" name="roleBean.name" id="name" value="${roleBean.name}"/> 
			...
			<table id="table1" width="100%" height="100%"  border="0" cellspacing="0" cellpadding="0"> 
			   	 <thead>
			   		<tr>  
			   			...
			   			<th sortname="name" width="90%">职位名称</th>	   
			   		</tr> 
			   	</thead>
			   	<tbody> 
