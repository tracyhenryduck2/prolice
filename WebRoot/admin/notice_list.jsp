...
<html>    
  <head>  
	...
	<script type="text/javascript">    
		function add(){      
			Dialog.open({Title:"新增", Width:1000, Height:560, URL:"<%=path%>/Notice!toAddNotice.action"});   
		}      
		function mod(){      
			...
			Dialog.open({Title:"编辑", Width:1000, Height:560, URL:"<%=path%>/Notice!toAddNotice.action?oper=1&noticeBean.id="+id});
		}      
		function detail(){   
			var id = getCheckRowValue();     
			if(id==null) {     
			...
			Dialog.open({Title:"详情", Width:1000, Height:560, URL:"<%=path%>/Notice!toAddNotice.action?read=1&oper=1&noticeBean.id="+id});    
		}      
		function del(){      
			if(!isCheckAny("idArr")){      
			...
			Dialog.confirm("确定删除吗", function(){     
				...
				form1.action = "<%=path%>/Notice!delNotices.action";
				form1.target = "fram";
				form1.submit();  
		...
		function search() {  
			Dialog.open({Title:"查询", Width:650, Height:220, URL:"<%=path%>/police/Notice!search.action",OnLoad:function(){
			     ...
			     this.innerDoc.getElementById("file_url").value = $("#file_url").val();
           }
			});    
		}      
  ...
  <body>  
  <GF:BodyCaption label="通知公告" ico="images/ico/user.gif"> 
  	<GF:ToolBar id="123">     
    ...
    <form action="<%=path%>/Notice!list.action" name="form1" id="form1" method="get">  
   
			<input type="hidden" name="noticeBean.title" id="title" value="${noticeBean.title}"/> 
			<input type="hidden" name="noticeBean.content" id="content" value="${noticeBean.content}"/> 
			<input type="hidden" name="noticeBean.date" id="date" value="${noticeBean.date}"/> 
			<input type="hidden" name="noticeBean.fileUrl" id="fileUrl" value="${noticeBean.fileUrl}"/> 
			...
			<table id="table1" width="100%" height="100%"  border="0" cellspacing="0" cellpadding="0"> 
			   	 <thead>
			   		<tr>  
			   			...
			   			<th sortname="content" width="10%">内容</th>	   
			   			<th sortname="date" width="10%">日期</th>	   
			   			<th sortname="file_url" width="10%">附件</th>	   
			   		</tr> 
			   	</thead>
			   	<tbody> 
			   		<s:iterator value="#request.list" id="map"> 
		   			 <tr> 
		   			 	 ...
		   			 	 <td>${map.title}</td>    
		   			 	 <td>${map.content}</td>    
		   			 	 <td><GF:ConvertTime value="${map.date}" format="yyyy-MM-dd HH:mm:ss"/></td>    
		   			 	 <td>${map.file_url}</td>    
		   			 </tr>
		   			 </s:iterator> 	    
		   		</tbody>		    		  
